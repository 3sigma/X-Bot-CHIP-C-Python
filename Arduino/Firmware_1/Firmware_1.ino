/******************************************************************************
Firmware de l'Iteaduino Uno du robot X-Bot
à base de CHIP, disponible à l'adresse:
...
void setMotors(int16_t left, int16_t right) {
  if (left >= 0) {
    ...
    analogWrite(pwmMoteurGauche, left);
  }
  if (left < 0) {
  ...
  if (right >= 0) {
    ...
    analogWrite(pwmMoteurDroit, right);
  }
  if (right < 0) {
...
void checkCommands() {
  ...
  slave.checkCommand(10, setMotors);
}
