...
def CalculVitesse():
    global started, omegaDroit, omegaGauche, \
        tdebut, ad, P_vx, I_vx, D_vx, P_xi, I_xi, D_xi, bd, Ti, yprecvx, yprecxi, \
        codeurDroitDeltaPos, codeurGaucheDeltaPos, commandeDroit, commandeGauche, vxmes, ximes, vxref, xiref, \
        distance, idecimDistance, codeurDroitDeltaPosPrec, codeurGaucheDeltaPosPrec, \
...
def CommandeMoteurs(commandeDroit, commandeGauche, tensionAlim):
    ...
    # Commande PWM
    try:
        uno.motors(tension_int_gauche, tension_int_droit)    
    except:
...
class WSHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        global x1, x2, Kpvx, Kivx, Kdvx, Kpxi, Kixi, Kdxi
        ...
        if jsonMessage.get('Kivx') != None:
            ...
            #print ("Kivx: %.2f" % Kivx)
        if jsonMessage.get('Kdvx') != None:
            Kdvx = float(jsonMessage.get('Kdvx'))
            #print ("Kdvx: %.2f" % Kdvx)
        if jsonMessage.get('Kpxi') != None:
            Kpxi = float(jsonMessage.get('Kpxi'))
            #print ("Kpxi: %.2f" % Kpxi)
        if jsonMessage.get('Kixi') != None:
            Kixi = float(jsonMessage.get('Kixi'))
            #print ("Kixi: %.2f" % Kixi)
        if jsonMessage.get('Kdxi') != None:
            Kdxi = float(jsonMessage.get('Kdxi'))
            #print ("Kdxi: %.2f" % Kdxi)
    ...
    def sendToSocket(self):
        global started, codeurDroitDeltaPos, codeurGaucheDeltaPos, socketOK, commandeDroit, commandeGauche, vxref, \
            xiref, vxmes, ximes, tensionAlim, distance, omegaDroit, omegaGauche
